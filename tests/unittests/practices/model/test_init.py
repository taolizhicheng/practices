import os
import sys


sys.dont_write_bytecode = True

MODULE_DIR = os.environ.get('MODULE_DIR', None)
if MODULE_DIR is not None:
    if not sys.path or sys.path[0] != MODULE_DIR:
        sys.path = [MODULE_DIR] + sys.path
else:
    raise ValueError("MODULE_DIR environment variable not set!")


import unittest
import practices.model
from practices.utils.build import get_builder


class TestModel(unittest.TestCase):
    def test_model_indices(self):
        builder = get_builder("MODEL_BUILDER")
        self.assertTrue("BaseModel" in builder)
        self.assertRaises(NotImplementedError, builder.build, "BaseModel", {})


if __name__ == "__main__":
    unittest.main()
